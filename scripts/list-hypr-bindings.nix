...
let
  inherit (import ../hosts/${host}/variables.nix)  programoptions;
in
pkgs.writeShellScriptBin "list-hypr-bindings" ''
  ...
  " + ENTER" "Terminal" "${programoptions.terminal}" \
  "ALT(MACRO) + SHIFT" "Change Keyboard Layout" ".." \
  ...
  " + W" "Launch Web Browser" "${programoptions.browser}" \
  " + E" "Launch Emoji Selector" "emopicker9000" \
  " + S" "Take Screenshot" "screenshootin" \
  " + D" "Launch ${programoptions.discord-client}" "${programoptions.discord-client}" \
  " + O" "Launch OBS" "obs" \
  " + G" "Launch GIMP" "gimp" \
